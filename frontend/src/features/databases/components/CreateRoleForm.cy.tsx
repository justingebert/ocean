...
describe("CreateRoleForm Component", () => {
  ...
  beforeEach(() => {
    ...
    cy.intercept("POST", "/v1/roles/_availability_", (req) => {
      ...
      if (instanceId === mockDatabase.id && roleName === `${mockDatabase.name}_validrole`) {
  ...
  it("submits an available role with its database prefix", () => {
    cy.get("input#roleName").type("validrole");
    ...
    cy.get("@onSubmit").should("have.been.calledWith", {
      ...
      roleName: `${mockDatabase.name}_validrole`,
    });
  });
